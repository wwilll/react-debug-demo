...
/* */
16384; // Union of side effect groupings as pertains to subtreeTag
...
var TOP_BEFORE_BLUR = unsafeCastStringToDOMTopLevelType('beforeblur');
var TOP_FOCUS_IN = unsafeCastStringToDOMTopLevelType('focusin');
var TOP_FOCUS_OUT = unsafeCastStringToDOMTopLevelType('focusout');
...
var discreteEventPairsForSimpleEventPlugin = [TOP_CANCEL, 'cancel', TOP_CLICK, 'click', TOP_CLOSE, 'close', TOP_CONTEXT_MENU, 'contextMenu', TOP_COPY, 'copy', TOP_CUT, 'cut', TOP_AUX_CLICK, 'auxClick', TOP_DOUBLE_CLICK, 'doubleClick', TOP_DRAG_END, 'dragEnd', TOP_DRAG_START, 'dragStart', TOP_DROP, 'drop', TOP_FOCUS_IN, 'focus', TOP_FOCUS_OUT, 'blur', TOP_INPUT, 'input', TOP_INVALID, 'invalid', TOP_KEY_DOWN, 'keyDown', TOP_KEY_PRESS, 'keyPress', TOP_KEY_UP, 'keyUp', TOP_MOUSE_DOWN, 'mouseDown', TOP_MOUSE_UP, 'mouseUp', TOP_PASTE, 'paste', TOP_PAUSE, 'pause', TOP_PLAY, 'play', TOP_POINTER_CANCEL, 'pointerCancel', TOP_POINTER_DOWN, 'pointerDown', TOP_POINTER_UP, 'pointerUp', TOP_RATE_CHANGE, 'rateChange', TOP_RESET, 'reset', TOP_SEEKED, 'seeked', TOP_SUBMIT, 'submit', TOP_TOUCH_CANCEL, 'touchCancel', TOP_TOUCH_END, 'touchEnd', TOP_TOUCH_START, 'touchStart', TOP_VOLUME_CHANGE, 'volumeChange'];
...
function registerEvents() {
  registerTwoPhaseEvent('onBeforeInput', [TOP_COMPOSITION_END, TOP_KEY_PRESS, TOP_TEXT_INPUT, TOP_PASTE]);
  registerTwoPhaseEvent('onCompositionEnd', [TOP_COMPOSITION_END, TOP_FOCUS_OUT, TOP_KEY_DOWN, TOP_KEY_PRESS, TOP_KEY_UP, TOP_MOUSE_DOWN]);
  registerTwoPhaseEvent('onCompositionStart', [TOP_COMPOSITION_START, TOP_FOCUS_OUT, TOP_KEY_DOWN, TOP_KEY_PRESS, TOP_KEY_UP, TOP_MOUSE_DOWN]);
  registerTwoPhaseEvent('onCompositionUpdate', [TOP_COMPOSITION_UPDATE, TOP_FOCUS_OUT, TOP_KEY_DOWN, TOP_KEY_PRESS, TOP_KEY_UP, TOP_MOUSE_DOWN]);
...
function extractCompositionEvent(dispatchQueue, topLevelType, targetInst, nativeEvent, nativeEventTarget) {
  var eventType;
  var fallbackData;
...
function extractBeforeInputEvent(dispatchQueue, topLevelType, targetInst, nativeEvent, nativeEventTarget) {
  var chars;
...
function extractEvents(dispatchQueue, topLevelType, targetInst, nativeEvent, nativeEventTarget, eventSystemFlags, targetContainer) {
  extractCompositionEvent(dispatchQueue, topLevelType, targetInst, nativeEvent, nativeEventTarget);
  extractBeforeInputEvent(dispatchQueue, topLevelType, targetInst, nativeEvent, nativeEventTarget);
}
...
function registerEvents$1() {
  registerTwoPhaseEvent('onChange', [TOP_CHANGE, TOP_CLICK, TOP_FOCUS_IN, TOP_FOCUS_OUT, TOP_INPUT, TOP_KEY_DOWN, TOP_KEY_UP, TOP_SELECTION_CHANGE]);
}
...
function extractEvents$1(dispatchQueue, topLevelType, targetInst, nativeEvent, nativeEventTarget, eventSystemFlags, targetContainer) {
...
function extractEvents$2(dispatchQueue, topLevelType, targetInst, nativeEvent, nativeEventTarget, eventSystemFlags, targetContainer) {
...
var rootTargetDependencies = [TOP_FOCUS_OUT, TOP_CONTEXT_MENU, TOP_DRAG_END, TOP_FOCUS_IN, TOP_KEY_DOWN, TOP_KEY_UP, TOP_MOUSE_DOWN, TOP_MOUSE_UP];

function registerEvents$3() {
  registerTwoPhaseEvent('onSelect', [].concat(rootTargetDependencies, [TOP_SELECTION_CHANGE]));
}
...
function extractEvents$3(dispatchQueue, topLevelType, targetInst, nativeEvent, nativeEventTarget, eventSystemFlags, targetContainer) {
  ...
  if ( // If we are handling TOP_SELECTION_CHANGE, then we don't need to
  ...
  topLevelType !== TOP_SELECTION_CHANGE && !eventListenerMap.has('onSelect') && !eventListenerMap.has('onSelectCapture')) {
    return;
  }
...
function extractEvents$4(dispatchQueue, topLevelType, targetInst, nativeEvent, nativeEventTarget, eventSystemFlags, targetContainer) {
  var reactName = topLevelEventsToReactNames.get(topLevelType);
...

  {
    ...
    var isNonDelegatedEvent = (eventSystemFlags & IS_NON_DELEGATED) !== 0; // TODO: We may also want to re-use the accumulateTargetOnly flag to
    // special case bubbling for onScroll/media events at a later point.

    var accumulateTargetOnly = inCapturePhase && isNonDelegatedEvent; // We traverse only capture or bubble phase listeners
...
function extractEvents$5(dispatchQueue, topLevelType, targetInst, nativeEvent, nativeEventTarget, eventSystemFlags, targetContainer) {
  ...
  extractEvents$4(dispatchQueue, topLevelType, targetInst, nativeEvent, nativeEventTarget, eventSystemFlags);
  ...
  if (shouldProcessPolyfillPlugins) {
    extractEvents$2(dispatchQueue, topLevelType, targetInst, nativeEvent, nativeEventTarget, eventSystemFlags);
    extractEvents$1(dispatchQueue, topLevelType, targetInst, nativeEvent, nativeEventTarget);
    extractEvents$3(dispatchQueue, topLevelType, targetInst, nativeEvent, nativeEventTarget, eventSystemFlags, targetContainer);
    extractEvents(dispatchQueue, topLevelType, targetInst, nativeEvent, nativeEventTarget);
  }
} // List of events that need to be individually attached to media elements.


var mediaEventTypes = [TOP_ABORT, TOP_CAN_PLAY, TOP_CAN_PLAY_THROUGH, TOP_DURATION_CHANGE, TOP_EMPTIED, TOP_ENCRYPTED, TOP_ENDED, TOP_ERROR, TOP_LOADED_DATA, TOP_LOADED_METADATA, TOP_LOAD_START, TOP_PAUSE, TOP_PLAY, TOP_PLAYING, TOP_PROGRESS, TOP_RATE_CHANGE, TOP_SEEKED, TOP_SEEKING, TOP_STALLED, TOP_SUSPEND, TOP_TIME_UPDATE, TOP_VOLUME_CHANGE, TOP_WAITING]; // We should not delegate these events to the container, but rather
// set them on the actual target element itself. This is primarily
// because these events do not consistently bubble in the DOM.

var nonDelegatedEvents = new Set([TOP_SCROLL, TOP_LOAD, TOP_CANCEL, TOP_CLOSE, TOP_INVALID].concat(mediaEventTypes));
...
function dispatchEventsForPlugins(topLevelType, eventSystemFlags, nativeEvent, targetInst, targetContainer) {
  ...
  var dispatchQueue = [];
  extractEvents$5(dispatchQueue, topLevelType, targetInst, nativeEvent, nativeEventTarget, eventSystemFlags, targetContainer);
...
function listenToNonDelegatedEvent(topLevelType, targetElement) {
  var isCapturePhaseListener = false;
  var listenerMap = getEventListenerMap(targetElement);
  var listenerMapKey = getListenerMapKey(topLevelType, isCapturePhaseListener);
  var listenerEntry = listenerMap.get(listenerMapKey);

  if (listenerEntry === undefined) {
    var listener = addTrappedEventListener(targetElement, topLevelType, PLUGIN_EVENT_SYSTEM | IS_NON_DELEGATED, isCapturePhaseListener);
...
function listenToNativeEvent(topLevelType, isCapturePhaseListener, rootContainerElement, targetElement, isPassiveListener, listenerPriority) {
  var eventSystemFlags = arguments.length > 6 && arguments[6] !== undefined ? arguments[6] : PLUGIN_EVENT_SYSTEM;
  var target = rootContainerElement; // TOP_SELECTION_CHANGE needs to be attached to the document
  ...
  if (listenerEntry === undefined || shouldUpgrade) {
    ...
    if (shouldUpgrade) {
      removeTrappedEventListener(target, topLevelType, isCapturePhaseListener, listenerEntry.listener);
    }
    ...
    var listener = addTrappedEventListener(target, topLevelType, eventSystemFlags, isCapturePhaseListener, false, isPassiveListener, listenerPriority);
    listenerMap.set(listenerMapKey, {
...
function listenToReactEvent(reactEvent, rootContainerElement, targetElement) {
  ...
  if (isPolyfillEventPlugin) {
    ...
    var isCapturePhaseListener = reactEvent.substr(-7) === 'Capture';
    listenToNativeEvent(dependencies[0], isCapturePhaseListener, rootContainerElement, targetElement);
  }
}

function addTrappedEventListener(targetContainer, topLevelType, eventSystemFlags, isCapturePhaseListener, isDeferredListenerForLegacyFBSupport, isPassiveListener, listenerPriority) {
  var listener = createEventListenerWrapperWithPriority(targetContainer, topLevelType, eventSystemFlags, listenerPriority); // If passive option is not supported, then the event will be
...
function accumulateSinglePhaseListeners(targetFiber, dispatchQueue, event, inCapturePhase, accumulateTargetOnly) {
  ...
  var listeners = []; // If we are not handling EventTarget only phase, then we're doing the
...
var discreteReplayableEvents = [TOP_MOUSE_DOWN, TOP_MOUSE_UP, TOP_TOUCH_CANCEL, TOP_TOUCH_END, TOP_TOUCH_START, TOP_AUX_CLICK, TOP_DOUBLE_CLICK, TOP_POINTER_CANCEL, TOP_POINTER_DOWN, TOP_POINTER_UP, TOP_DRAG_END, TOP_DRAG_START, TOP_DROP, TOP_COMPOSITION_END, TOP_COMPOSITION_START, TOP_KEY_DOWN, TOP_KEY_PRESS, TOP_KEY_UP, TOP_INPUT, TOP_TEXT_INPUT, TOP_COPY, TOP_CUT, TOP_PASTE, TOP_CLICK, TOP_CHANGE, TOP_CONTEXT_MENU, TOP_RESET, TOP_SUBMIT];
var continuousReplayableEvents = [TOP_DRAG_ENTER, TOP_DRAG_LEAVE, TOP_FOCUS_IN, TOP_FOCUS_OUT, TOP_MOUSE_OVER, TOP_MOUSE_OUT, TOP_POINTER_OVER, TOP_POINTER_OUT, TOP_GOT_POINTER_CAPTURE, TOP_LOST_POINTER_CAPTURE];
function isReplayableDiscreteEvent(eventType) {
  return discreteReplayableEvents.indexOf(eventType) > -1;
}

function trapReplayableEventForContainer(topLevelType, container) {
  listenToNativeEvent(topLevelType, false, container, null);
}
...
function dispatchUserBlockingUpdate(topLevelType, eventSystemFlags, container, nativeEvent) {
  ...
  try {
    ...
    runWithPriority(UserBlockingPriority$1, dispatchEvent.bind(null, topLevelType, eventSystemFlags, container, nativeEvent));
  } finally {
...
function setInitialProperties(domElement, tag, rawProps, rootContainerElement) {
  ...
  switch (tag) {
    ...
    case 'input':
      ...
      listenToNonDelegatedEvent(TOP_INVALID, domElement); // For controlled components we always need to ensure we're listening
...
function ensureRootIsScheduled(root, currentTime) {
  ...
  var newCallbackId = getNextLanes(root, root === workInProgressRoot ? workInProgressRootRenderLanes : NoLanes); // This returns the priority level computed during the `getNextLanes` call.
  ...
  var existingCallbackId = root.callbackId;
...
function performConcurrentWorkOnRoot(root, didTimeout) {
  ...
  flushPassiveEffects(); // Determine the next expiration time to work on, using the fields stored
  // on the root.
...
function commitRootImpl(root, renderPriorityLevel) {
  ...
  root.callbackNode = null;
  root.callbackId = NoLanes; // Update the first and last pending times on this root. The new first
